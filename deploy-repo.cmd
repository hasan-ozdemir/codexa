...
powershell -NoProfile -Command "$ErrorActionPreference='Stop';$v='%RELEASE_VERSION%';$root='%REPO_ROOT%';$jsons=@('codex-cli/package.json','sdk/typescript/package.json','codex-rs/responses-api-proxy/npm/package.json','shell-tool-mcp/package.json');foreach($rel in $jsons){$p=Join-Path $root $rel;$obj=Get-Content $p -Raw | ConvertFrom-Json;$obj.version=$v;$obj | ConvertTo-Json -Depth 50 | Set-Content -Encoding UTF8 $p};$tomlPath=Join-Path $root 'codex-rs/Cargo.toml';$content=Get-Content $tomlPath -Raw;$pattern='(?m)^version\\s*=\\s*\".*\"$';if(-not ($content -match $pattern)){throw 'workspace.package version not found'};$new=[regex]::Replace($content,$pattern,('version = \"'+$v+'\"'),1);Set-Content -Path $tomlPath -Value $new -Encoding UTF8;"
if errorlevel 1 (
