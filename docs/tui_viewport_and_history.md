...
This document describes the design in terms of the original TUI crate. The TUI2 implementation
(`codex-rs/tui2`) applies the same model:

- The viewport and transcript are owned entirely by the TUI2 app (`codex-rs/tui2/src/app.rs`),
  using `transcript_cells`, `TranscriptScroll`, and `TranscriptSelection` to track scroll and
  selection state.
- Inline user prompts are rendered with row‑wide padding and user background styling in both the
  live viewport and the printed exit transcript.
- Mouse and keyboard scrolling share a common scroll/anchor model; wheel scrolling clears
  selections, while keyboard PgUp/PgDn/Home/End preserve selection state.
- Exit transcripts and suspend‑time history printing follow the same append‑only, cell‑based
  high‑water‑mark strategy described above, with the TUI2‑specific wiring documented in
  `codex-rs/docs/tui2_viewport_and_history.md`.
